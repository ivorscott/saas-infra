...
OIDC_PROVIDER_URL=aws eks describe-cluster --name $(terraform output -raw eks_cluster_name) --query "cluster.identity.oidc.issuer" --output text
AWS_ACCOUNT_ID=aws sts get-caller-identity --query "Account" --output text --profile ivorscott
AWS_REGION=aws configure get region --profile ivorscott
...
aws iam create-policy --policy-name AmazonEKS_EBS_CSI_Driver_Policy --policy-document file://policies/ebs-iam-policy.json

# Create the IAM trust policy file
cat <<EOF > policies/trust-policy.json
{
  "Version": "2012-10-17",
  "Statement": [
    {
